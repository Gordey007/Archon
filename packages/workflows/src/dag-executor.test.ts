...
describe('executeDagWorkflow -- node-level retry for transient errors', () => {
  ...
  it('node retries when SDK reports network connection lost', async () => {
    let callCount = 0;
    mockSendQueryDag.mockImplementation(function* () {
      callCount++;
      if (callCount === 1) {
        yield {
          ...
          stopReason: 'error',
          sessionId: 'net-lost-sess',
        };
        return;
      }
      yield { type: 'assistant', content: 'Recovered after reconnect' };
      yield { type: 'result', sessionId: 'retry-sess' };
    });

    const mockDeps = createMockDeps();
    const platform = createMockPlatform();
    const workflowRun = makeWorkflowRun('dag-retry-network-lost-run');
    ...
    await executeDagWorkflow(
      mockDeps,
      platform,
      'conv-dag-retry-network-lost',
      testDir,
      { name: 'dag-retry-network-lost', nodes },
      workflowRun,
      'claude',
      undefined,
      join(testDir, 'artifacts'),
